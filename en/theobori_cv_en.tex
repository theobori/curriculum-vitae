...
\begin{resume}
  ...
  {\bf Infrastructure and Services}
  \hfill {\it September 2022--September 2026}
  \vspace*{.05in}
  \begin{itemize}
    \item Provision of free services open to everyone. All services
      are containerized using Docker and accessible behind NGINX,
      with deployment automated using an Ansible playbook and DNS
      records managed through a Terraform project. Prometheus and
      Grafana are used to monitor the health of the services and
      system, and automatic backups are performed weekly.
      Authentication to the services is managed by an OpenLDAP
      server, with blocking mechanisms in place to prevent malicious
      authentication attempts. All technical documentation and
      user statistics are publicly available. The infrastructure
      was decommissioned in September 2026, while all associated
      configurations and automation remain publicly available in the
      dedicated
      \href{https://github.com/theobori-cafe}{GitHub organization}.
    \item Transformation of an old machine into a
      \href{https://github.com/theobori/homelab}{personal homelab},
      with the goal of giving unused computer hardware a second life.
      It has thus become a private platform where all identities and
      access are managed by Authentik, and all services are exposed
      through Traefik. It includes automatic backups, Grafana and
      Uptime Kuma alerts, as well as a collection of services for
      managing media and files. Most of the technical details are
      documented on
      \href{https://theobori.cafe/posts/homelab/}{this webpage}.
  \end{itemize}
  ...
  {\bf Gopher Protocol}
  \hfill {\it June 2026--July 2026}
  \vspace*{.05in}
  \begin{itemize}
    \item Development of a
      \href{https://github.com/theobori/fleur}{framework} in Go
      for building highly customized Gopher applications compatible
      with the RFC 1436 standard and beyond. It comes with a CLI that
      implements a standard application using the framework, as well
      as a NixOS module for deploying it as a service.
    \item Development of a
      \href{https://github.com/theobori/fleur-form}{Go extension}
      for the aforementioned framework, adding support for creating
      user-facing forms.
    \item Development of a
      \href{https://github.com/theobori/ange}{Gopher application} in
      Go using the two aforementioned projects. It provides a
      persistent Gopher webring, allowing users to add, remove, and
      validate Gopher sites wishing to join the chain. It also allows
      retrieving the neighboring nodes of a given node and selecting
      a node at random.
  \end{itemize}
